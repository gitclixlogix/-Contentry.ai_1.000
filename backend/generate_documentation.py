#!/usr/bin/env python3
"""
Generate updated platform documentation with current system statistics
"""
import asyncio
...
async def get_system_stats():
    ...
    stats = {
        ...
        'flagged_posts': await db.posts.count_documents({'flagged_status': {'$in': ['policy_violation', 'harassment']}}),
    }
    
    # Get enterprise names
    enterprises = await db.enterprises.find({}, {'_id': 0, 'name': 1, 'domains': 1}).to_list(100)
...
async def generate_documentation():
    ...
    doc_content = f"""# Contentry AI Platform Documentation
...
**Current Platform Status (as of {current_date}):**
- **Total Users:** {stats['total_users']}
- **Total Enterprises:** {stats['total_enterprises']}
...
"""
    ...
    if stats['enterprises']:
        for ent in stats['enterprises']:
            doc_content += f"\n- **{ent['name']}** (Domains: {', '.join(ent['domains'])})"
    else:
...
"""
    ...
    # Write to file
    doc_path = ROOT_DIR.parent / "CONTENTRY_PLATFORM_DOCUMENTATION.md"
    ...
    print(f"   Stats: {stats['total_users']} users, {stats['total_enterprises']} enterprises, {stats['total_posts']} posts")
